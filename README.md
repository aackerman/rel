...
The type of input for `AttributeNode#Eq` is not predictable. In some cases a user may want to use an `int`, `string`, or another `AttributeNode`. When an input type is unpredictable Rel uses the `Visitable` type for input as opposed to an empty interface, and offers the `Sql` method as a way to convert primitive types to a value that will satisfy the `Visitable` interface.
